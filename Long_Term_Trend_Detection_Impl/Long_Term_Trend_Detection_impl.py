...
def generate_ngrams(text):
    n_grams = ngrams(nltk.word_tokenize(text), 3)#used to change ngrams
...
def find_all_clusters(keywords, scores):
    ...
    cluster = AgglomerativeClustering(n_clusters=None, affinity='euclidean', linkage='average', distance_threshold=0.75)
    cluster.fit(keyword_score)
    # labels = cluster.n_clusters_
...
def filterDataOnDate():
    ...
    out = df[df['created_at'].between('2021-09-02', '2021-09-17')]
    return out;
...
def sportTrendsJson():
    ...
    df["Category"] = "Sports"
    df.to_csv("outputSportsFinalUni.csv", index=False)
    with open('outputSportsFinalUni.csv', encoding='utf-8') as f:
...
def politicTrendsJson():
    ...
    df.to_csv("outputPoliticsFinalUni.csv", index=False)
    with open('outputPoliticsFinalUni.csv', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        rows = list(reader)
...
def entertainmentTrendsJson():
    ...
    df.to_csv("outputEntertainmentFinalUni.csv", index=False)
    with open('outputEntertainmentFinalUni.csv', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        rows = list(reader)
...
def crimeTrendsJson():
    ...
    df = df.sort_values(by='Date')
    df["Category"] = "Crime"
    df.to_csv("outputCrimeFinalUni.csv", index=False)
    with open('outputCrimeFinalUni.csv', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        rows = list(reader)
...
def otherTrendsJson():
    ...
    df = df.sort_values(by='Date')
    df["Category"] = "Other"
    df.to_csv("outputOtherFinalUni.csv", index=False)
    with open('outputOtherFinalUni.csv', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        rows = list(reader)
